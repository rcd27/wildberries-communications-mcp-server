...
export const GetArchivedFeedbacksQuerySchema = z.object({
  ...
  take: z.number().int().min(1).max(5000).describe('Количество отзывов (max. 5 000)'),
  skip: z.number().int().min(0).describe('Количество отзывов для пропуска'),
  order: z.enum(['dateAsc', 'dateDesc']).optional().describe(
...
export type GetArchivedFeedbacksQuery = z.infer<typeof GetArchivedFeedbacksQuerySchema>;

export const FeedbackAnswerSchema =
  z.object({
...
export const ProductDetailsSchema =
  z.object({
     imtId: z.number().int().optional().describe('ID карточки товара (imtId)'),
     nmId: z.number().int().optional().describe('Артикул WB (nmId)'),
     productName: z.string().optional().describe('Название товара'),
     supplierArticle: z.string().nullable().optional().describe('Артикул продавца (nullable)'),
     supplierName: z.string().nullable().optional().describe('Имя продавца (nullable)'),
     brandName: z.string().nullable().optional().describe('Бренд товара (nullable)'),
     size: z.string().optional().describe('Размер товара (`techSize` в КТ)')
   })
   .describe('Структура товара');

export const PhotoLinkSchema =
  z.object({
...
export const FeedbackSchema = z.object({
  ...
  productValuation: z.number().int().optional().describe('Оценка товара (целое число)'),
  createdDate: z.string().datetime().optional().describe('Дата и время создания отзыва (ISO 8601)'),
  answer: FeedbackAnswerSchema,
  state: z.enum(['none', 'wbRu']).optional().describe('Статус отзыва: `none` — не обработан, `wbRu` — обработан'),
  productDetails: ProductDetailsSchema.optional().describe('Детали товара (если доступны)'),
  photoLinks: z.array(PhotoLinkSchema).nullable().optional().describe('Массив структур фотографий (nullable)'),
  video: VideoSchema,
  ...
  isAbleSupplierFeedbackValuation: z.boolean().optional().describe(
    'Доступна ли продавцу возможность оставить жалобу на отзыв (true/false)'),
  supplierFeedbackValuation: z.number().int().optional().describe('Ключ причины жалобы на отзыв (если применимо)'),
  isAbleSupplierProductValuation: z.boolean().optional().describe(
    'Доступна ли продавцу возможность сообщить о проблеме с товаром (true/false)'),
  supplierProductValuation: z.number().int().optional().describe('Ключ проблемы с товаром (если применимо)'),
  isAbleReturnProductOrders: z.boolean()
                              .optional()
                              .nullable()
  ...
  bables: z.array(z.string()).nullable()
           .optional()
...
export const GetArchivedFeedbacksResponseSchema = createResponseSchema(z.object({
  feedbacks: z.array(FeedbackSchema)
}));

export type GetArchivedFeedbacksResponse = z.infer<typeof GetArchivedFeedbacksResponseSchema>;
